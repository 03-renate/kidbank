...
function addAmount(amount) {
  if (amount > 0) {
    ...
    console.log(
      `Added ${amount}${bankAccount.symbol}. New balance: ${bankAccount.balance}${bankAccount.symbol}`
    );
    renderAccount();
  } else {
...
function subtractAmount(amount) {
  if (amount > 0 && amount <= bankAccount.balance) {
    bankAccount.balance -= amount;
    console.log(
      `Withdrew ${amount}${bankAccount.symbol}. New balance: ${bankAccount.balance}${bankAccount.symbol}`
    );
    renderAccount();
  } else {
    console.error(
      "Invalid amount. Ensure the amount is positive and does not exceed the balance."
    );
  }
}
...
function renderAccount() {
  ...
  accountDiv.innerHTML = `<h2>Account Overview</h2>
